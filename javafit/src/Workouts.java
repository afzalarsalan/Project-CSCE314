...
public class Workouts {
	...
	public final Workouts getWorkoutsByMuscle(Muscle m, boolean includeSecondary) {
		...
		Workouts returnWorkouts = new Workouts();
		for (Workout exercise : workoutList) {
			if(exercise.primaryMuscle == m) {
				returnWorkouts.addWorkout(exercise);
			} else if (includeSecondary && exercise.secondaryMuscle == m) {
				returnWorkouts.addWorkout(exercise);
			}
		}
	...
	public final Workouts getWorkoutsByEquipment(ArrayList<Equipment> e) {
		Workouts returnWorkouts = new Workouts();
		for (Workout exercise : workoutList) {
			for (Equipment workoutEquipment : e) {
				if(exercise.equipment == workoutEquipment) {
					returnWorkouts.addWorkout(exercise);
					break;
				}
			}
	...
	public final ArrayList<String[]> getFullInformation() {
		...
		for(Workout exercise : workoutList) {
			workoutInfo.add(new String[]{exercise.name, exercise.equipment.name(), exercise.primaryMuscle.name(), exercise.secondaryMuscle.name(),
			                 exercise.desc, exercise.reminders});
		}
		return workoutInfo;
	}
}
